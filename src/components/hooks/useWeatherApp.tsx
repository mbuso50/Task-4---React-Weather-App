...
export const useWeatherApp = () => {
  ...
  useEffect(() => {
    if (currentLocation) {
      fetchWeatherData(currentLocation, settings.unit);
    }
  ...
  useEffect(() => {
    if (weatherData && settings.notifications) {
      ...
      if (weatherData.current.wind_speed > 20) alerts.push("High wind warning");
      ...
      if (
        weatherData.current.weather.main === "Rain" &&
        weatherData.current.weather.description.includes("heavy")
      )
      ...
      if (alerts.length > 0 && "Notification" in window) {
        if (Notification.permission === "default") {
          Notification.requestPermission().then((permission) => {
            if (permission === "granted") showNotifications(alerts);
          });
  ...
  const fetchWeatherData = async (
    ...
    try {
      const [current, forecast] = await Promise.all([
        ...
        getForecast(location.lat, location.lon, units),
      ]);
